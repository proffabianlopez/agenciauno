...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (clients_exists($email_cliente)) {
        header("Location: ../views/printer.php?error=El cliente ya existe");
    } else {
        ...
        if ($id_cliente) {
            header("Location: ../views/printer.php?success=Cliente agregado correctamente");
        } else {
            header("Location: ../views/printer.php?error=Error al agregar el cliente");
        }
    }
}
